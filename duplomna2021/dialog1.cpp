...
void Dialog1::on_pushButton_7_clicked()
{
    ...
    query.prepare ( "INSERT INTO  Bayes (Result_1) VALUES (:Result_1)");
    QString str1 = ui-> lineEdit_database-> text ();
...
void Dialog1::on_pushButton_6_clicked()
{
    ...
    std :: vector < QLineEdit *> lineEdits_ = { ui->lineEdit_31, ui->lineEdit_32, ui->lineEdit_33 , ui->lineEdit_34, ui->lineEdit_35};
   // Створюємо вектор для збереження значень з поле введення
    std :: vector <double> values_ ;

    //ЙМОВІРНОСТІ
    QString S6 = ui->lineEdit_31->text();
    z1 = S6.toFloat();

    QString S7 = ui->lineEdit_32->text();
    z2 = S7.toFloat();

    QString S8 = ui->lineEdit_33->text();
    z3 = S8.toFloat();

    QString S9 = ui->lineEdit_34->text();
    z4 = S9.toFloat();

    QString S10 = ui->lineEdit_35->text();
    z5 = S10.toFloat();
    ...
    for ( const QLineEdit * lineEdit : lineEdits_ )
        {
            ...
            if ( values_.empty ())
           {
               ...
               ui->lineEdit_5 -> setText ( "Nan" );
           }
            else{}

 //Зчитування даних з першої альтернативи A1
 ...
 std :: vector <double> values ;
     ...
     a5 = S5.toFloat();
       ...
       {
           //Рахуєм математичне очікування для першої альтернативи
           x=(a1*z1)+(a2*z2)+(a3*z3)+(a4*z4)+(a5*z5);
           //Виводимо результат
           ui->lineEdit_5->setText(QString::number(x));
       }

 //Зчитування даних з другої альтернативи A2
 ...
 std :: vector <double> values1 ;
     ...
     b5 = S15.toFloat();
       ...
       {
           //Рахуєм математичне очікування для другої альтернативи
           y=(b1*z1)+(b2*z2)+(b3*z3)+(b4*z4)+(b5*z5);
           //Виводимо результат
           ui->lineEdit_12->setText(QString::number(y));
       }

 //Зчитування даних з третьої альтернативи A3
 ...
 std :: vector <double> values2 ;
     ...
     c5 = S20.toFloat();
       ...
       {
           //Рахуєм математичне очікування для третьої альтернативи
           z=(c1*z1)+(c2*z2)+(c3*z3)+(c4*z4)+(c5*z5);
           //Виводимо результат
           ui->lineEdit_18->setText(QString::number(z));
       }
   //Зчитування даних з четверотої альтернативи A4
   ...
   std :: vector <double> values3 ;
       ...
       {   //Рахуєм математичне очікування для четвертої альтернативи
           n=(d1*z1)+(d2*z2)+(d3*z3)+(d4*z4)+(d5*z5);
           //Виводимо результат
           ui->lineEdit_24->setText(QString::number(n));}

  //Зчитування даних з п'ятої альтернативи A5
  ...
  std :: vector <double> values4 ;
       ...
       {   //Рахуєм математичне очікування для п'ятої альтернативи
           m=(i1*z1)+(i2*z2)+(i3*z3)+(i4*z4)+(i5*z5);
           //Виводимо результат
           ui->lineEdit_30->setText(QString::number(m));}
}

void Dialog1::on_pushButton_clicked()
{
    std :: vector < QLineEdit *> lineEdits5 = { ui->lineEdit_5, ui->lineEdit_12, ui->lineEdit_18 , ui->lineEdit_24, ui->lineEdit_30 };
    ...
    for ( const QLineEdit * lineEdit : lineEdits5 )
      {
          ...
          if (ok)
          {
           ...
           //Знайти максимальне значення засобами стандартної бібліотеки
           double max = * std :: max_element ( values5.begin(), values5.end ());
           ui -> lineEdit_database -> setText ( QString :: number ( max ));

           //Пошук найкращої альтернативи для інвестування
           if( values5[0] == max ){
               ui -> lineEdit_database -> setText ( "A1");
           }
           else if(values5[1] == max){
               ui -> lineEdit_database -> setText ( "A2");
           }
           else if(values5[2] == max){
               ui -> lineEdit_database -> setText ( "A3");
           }
           else if(values5[3] == max){
               ui -> lineEdit_database -> setText ( "A4");
           }
           else if(values5[4] == max){
               ui -> lineEdit_database -> setText ( "A5");
           }
}
